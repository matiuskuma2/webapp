...
| 商品名 | API モデル名 | 特徴 | 速度 | 品質 | **コスト/枚** |
|--------|-------------|------|------|------|-------------|
| **Nano Banana** | `gemini-2.5-flash-image` | Flash-tier、高速、安定 | 速い（~5s/枚） | ○ 良好 | **$0.039** |
| **Nano Banana Pro** | `gemini-3-pro-image-preview` | Thinking搭載、4K対応、高忠実テキスト | 遅い（~19s/枚） | ◎ 最高 | **$0.134** (1K/2K) / **$0.24** (4K) |

### 8.1.1 コスト比較（重要）

| 指標 | Nano Banana (Flash) | Nano Banana Pro | **倍率** |
|------|-------------------|-----------------|---------|
| 1枚あたり | $0.039 | $0.134 | **×3.4** |
| 5シーン（丸投げ） | $0.195 | $0.670 | **×3.4** |
| 20シーン（ボード） | $0.780 | $2.680 | **×3.4** |
| 4K 1枚 | N/A | $0.240 | — |

**現在の採用方針**: 両フロー（制作ボード + 丸投げ）とも **Nano Banana Pro** に統一。
品質優先だが、コスト管理が必要。`image_generation_logs` で全生成のコストを追跡中。

### 8.1.2 将来のモデル選択ガイドライン

| ユースケース | 推奨モデル | 理由 |
|-------------|-----------|------|
| 本番・最終品質 | Nano Banana Pro | 品質最優先 |
| プレビュー・ドラフト | Nano Banana (Flash) | コスト効率 |
| 大量バッチ生成 | Nano Banana (Flash) | RPM制限が緩い |
| 4K出力が必要 | Nano Banana Pro | Flash は 4K 非対応 |
