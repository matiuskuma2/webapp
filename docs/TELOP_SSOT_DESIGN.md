...
**Created**: 2026-02-01  
**Updated**: 2026-02-02  
**Status**: Phase 1 完了、Phase 2-1 完了、Phase 2-2 完了、Phase 2-3 完了、Phase 3-A 完了、PR-Comic-Rebake-All 完了、PR-Comic-Rebake-DiffBadge 完了、PR-Remotion-Subtitle-Typography 完了
...
漫画アセットには2つのレイヤーがあり、それぞれ別の処理が必要です。

| 用語 | 対象 | 処理内容 | コスト |
|------|------|----------|--------|
| **AI画像の再生成** | ベース画像（キャラ・背景） | 画像生成モデルを再実行 | 高（モデルコスト） |
| **漫画の再焼き込み** | 焼き込みレイヤー（吹き出し・テロップ） | Canvas描画で文字を再描画 | 低（クライアント処理） |
...
┌────────────────────────┐
│  焼き込みレイヤー        │  ← 「再焼き込み」対象
│  (吹き出し/テロップ)     │     telops_comic 設定で変わる
├────────────────────────┤
│  ベース画像              │  ← 「AI画像の再生成」対象
│  (キャラ/背景)           │     ❌ Phase2系では触らない
└────────────────────────┘
```
...
| ❌ 避けるべき表記 | ✅ 推奨表記 |
|------------------|------------|
| 「漫画を再生成」 | 「文字設定を反映」「再焼き込み」 |
| 「画像を再生成」（漫画コンテキスト） | 「AI画像を再生成」（明示的に区別） |
