...
CREATE TABLE image_generations (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, asset_type TEXT DEFAULT 'ai', started_at DATETIME, ended_at DATETIME, duration_ms INTEGER, gemini_duration_ms INTEGER, r2_duration_ms INTEGER, retry_count INTEGER NOT NULL DEFAULT 0,
  FOREIGN KEY (scene_id) REFERENCES scenes(id) ON DELETE CASCADE
);

CREATE TABLE marunage_runs (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE project_audio_jobs (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE "projects" (
  ...
  error_message TEXT,
  last_error DATETIME
, user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, is_template INTEGER DEFAULT 0, template_label TEXT, template_description TEXT, output_preset TEXT DEFAULT 'yt_long', split_mode TEXT DEFAULT 'raw', target_scene_count INTEGER DEFAULT 5, settings_json TEXT DEFAULT '{}', is_deleted INTEGER NOT NULL DEFAULT 0, deleted_at DATETIME);
